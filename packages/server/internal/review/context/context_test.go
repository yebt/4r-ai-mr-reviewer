...
func TestFastStrategyBuildsDiffInput(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"iid":7,"title":"Add login","description":"adds auth","source_branch":"feature/login","target_branch":"main","changes":[{"old_path":"a.go","new_path":"a.go","diff":"@@ hunk @@"}]}`))
	}))
	defer srv.Close()
	...
	if in.SourceBranch != "feature/login" || in.TargetBranch != "main" {
		t.Fatalf("branches not captured: source=%q target=%q", in.SourceBranch, in.TargetBranch)
	}
